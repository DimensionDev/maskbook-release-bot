...
export async function hotfix(context: Context<Webhooks.EventPayloads.WebhookPayloadPullRequest>) {
    ...
    const { head, base } = pr
    if (!(base.ref === 'released' && head.ref.startsWith('hotfix/'))) return
    ...
    const relatedPR = await context.octokit.pulls
        .list(context.repo({ state: 'open', head: `hotfix/${version}`, base: 'master' }))
        .then((x) => x.data.find((x) => x.title.includes(`2 of 2`))!)
        .then((x) => context.octokit.pulls.get(context.repo({ pull_number: x.number })))
        .then((x) => x.data)
